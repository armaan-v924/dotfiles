...
local function toggle_inlay_hints(bufnr)
	...
	local msg = inlay_hints_enabled and "Inlay hints enabled" or "Inlay hints disabled"
	if vim.notify then
		vim.notify(msg, vim.log.levels.INFO, { title = "Python LSP" })
	end
end
...
function M.setup()
	...
	if has_cmd("cargo") then
		...
		vim.schedule(function()
			if vim.notify then
				vim.notify(
					"Skipping rust-analyzer setup (cargo not found in PATH).",
					vim.log.levels.WARN,
					{ title = "LSP" }
				)
			end
	...
	-- Register Python-specific keybinds immediately on FileType
	vim.api.nvim_create_autocmd("FileType", {
		pattern = "python",
		callback = function(args)
			...
			end, { buffer = bufnr, silent = true, desc = "Toggle Python type hints" })
			...
			if ok then
				wk.add({
					...
					{ "<leader>pt", desc = "Toggle type hints", buffer = bufnr },
				}, { mode = "n" })
			end

			-- Enable inlay hints by default for Python files
