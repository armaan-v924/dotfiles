...
main() {
  ...
  local marker="generated-by-dotfiles"
  local machine_marker=""

  if [[ -f "$machine_file" ]]; then
    machine_marker="$(git config --file "$machine_file" dotfiles.marker 2>/dev/null || true)"
    if [[ "$machine_marker" == "$marker" ]]; then
      log::info "Existing git config generated by dotfiles; skipping Git setup."
      return 0
    fi
  fi
  ...
  write_kv "$machine_file" "dotfiles.marker" "$marker"
